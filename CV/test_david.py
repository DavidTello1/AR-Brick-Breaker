...
#------------------------------------------------------------------------
    # scale_percent = 50.0/ 100.0
    
    # # Resize Input (60%)
    # iH, iW = Input.shape[:2]
    # W = int(iW*scale_percent)
    # H = int(iH*scale_percent)
    # resized_input = cv2.resize(Input, (W, H), interpolation=cv2.INTER_AREA)

    # # Resize Target (60%)
    # th, tw = Target.shape[:2]
    # w = int(tw*scale_percent)
    # h = int(th*scale_percent)

    # resized_target = cv2.resize(Target, (w, h), interpolation=cv2.INTER_AREA)

    # # Input image GrayScale
    # input_gray = cv2.cvtColor(resized_input, cv2.COLOR_BGR2GRAY)
    # target_gray = cv2.cvtColor(resized_target, cv2.COLOR_BGR2GRAY)
