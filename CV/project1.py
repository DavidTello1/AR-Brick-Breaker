...
def test1():
    ...
    # cv2.normalize(inputImgGray, inputImgGray, alpha=0, beta=1, norm_type=cv2.NORM_MINMAX, dtype=cv2.CV_32F)
    # cv2.normalize(targetGray, targetGray, alpha=0, beta=1, norm_type=cv2.NORM_MINMAX, dtype=cv2.CV_32F)
   ...
   # matchMap = cv2.matchTemplate(inputImg, target, 1)
    ...
    # cv2.imshow("gray Image", inputImgGray)
    # cv2.imshow("Gray target", targetGray)
